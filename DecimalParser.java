...
class DecimalParser {
    ...
    enum StateDecimal {
        SSTART, SINTEGER, SDECIMAL, SEND
    }

    public static double myParseDecimal(String str) throws Exception {
        StateDecimal state = StateDecimal.SSTART;

        int i = 0;
        double divider = 10;
        double value = 0;

        while (i < str.length() && state != StateDecimal.SEND) {
